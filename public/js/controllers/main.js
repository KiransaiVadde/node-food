angular.module('foodController', [])
	...
	.controller('mainController', ['$scope','$http','Foods', function($scope, $http, Foods) {
		...
		// GET =====================================================================
		// when submitting the Display total price of all foods, gets the total 
		// and show them 
		// use the service to get total price of all foods
			
		$scope.getTotal = function(){
			Foods.getTotal()
			.success(function(response) {
			$scope.formTotal = response;
			$scope.$apply()
			});
		}			
		...
		$scope.createFood = function() {
			...
			if ($scope.formData.text != undefined) {
				...
				var obj = {name: $scope.formData.text, price: $scope.formPrice.text}
